...
begin
  Resultado:= '';
  CriaStr(Resultado, 130);
  with Dtm_DNF do
  begin
    Molda('0', '0', 1, 1, True, Resultado);
    Molda(Periodo, '0', 2, 7, True, Resultado);
    Molda('92661453000187', '0', 8, 21, True, Resultado);
    Molda(CDS_Filtro.FieldByName('NUMERO').AsString, '0', 22, 30, True, Resultado);
    Molda(CDS_Filtro.FieldByName('SERIE').AsString, ' ', 31, 33, False, Resultado);
    Molda(FormatDateTime('ddmmyyyy', CDS_Filtro.FieldByName('DATADOCUMENTO').AsDateTime), '0', 34, 41, True, Resultado);
    Molda(FormatDateTime('ddmmyyyy', CDS_Filtro.FieldByName('DATADOCUMENTO').AsDateTime), '0', 42, 49, True, Resultado);
    if CDS_Filtro.FieldByName('NumeroCGCMF').AsFloat <> 0 then
    Molda(TiraChar(CDS_Filtro.FieldByName('NUMEROCGCMF').AsString), '0', 50, 63, True, Resultado) else
    Molda(TiraChar(CDS_Filtro.FieldByName('NUMEROCPF').AsString), ' ', 50, 63, False, Resultado);
    Molda(' ', ' ', 64, 133, False, Resultado);
  end;
  Result:= Resultado;
//  Result:= Result + '0D0A';
end;
...
function DadosItens(Periodo: String; NumItem: Integer): String;
var
  Unidade, Resultado: String;
  ValorUnitario, ValorTotal, QuantEst: Extended;
begin
  Resultado:= '';
  CriaStr(Resultado, 147);
  with Dtm_DNF do
  begin
    ...
    Molda('1', '0', 1, 1, True, Resultado);
    Molda(Periodo, '0', 2, 7, True, Resultado);
    Molda('92661453000187', '0', 8, 21, True, Resultado);
    Molda(CDS_Filtro.FieldByName('NUMERO').AsString, '0', 22, 30, True, Resultado);
    Molda(CDS_Filtro.FieldByName('SERIE').AsString, ' ', 31, 33, False, Resultado);
    Molda(FormatDateTime('ddmmyyyy', CDS_Filtro.FieldByName('DATADOCUMENTO').AsDateTime), '0', 34, 41, True, Resultado);
    Molda(IntToStr(NumItem), '0', 42, 44, True, Resultado);
    Molda('095', '0', 45, 47, True, Resultado);
    Molda('0', '0', 48, 52, True, Resultado);
    Molda(CDS_MCliPro.FieldByName('CodFiscalPro').AsString, '0', 53, 56, True, Resultado);
    Molda(Unidade, ' ', 57, 86, False, Resultado);
    Molda(TiraChar(FormatFloat('0.000', CDS_MCliPro.FieldByName('QUANTATENDIDA').AsFloat)), '0', 87, 100, True, Resultado);
    Molda(TiraChar(FormatFloat('0.000000', ValorUnitario)), '0', 101, 116, True, Resultado);
    Molda(TiraChar(FormatFloat('0.00', ValorTotal)), '0', 117, 130, True, Resultado);
    Molda(TiraChar(FormatFloat('0.000', CDS_MCliPro.FieldByName('AliqIpi').AsFloat)), '0', 131, 136, True, Resultado);
    Molda(TiraChar(FormatFloat('0.000', QuantEst)), '0', 137, 150, True, Resultado);
  end;
  Result:= Resultado;
//  Result:= Result + '0D0A';
end;
...
begin
  Resultado:= '';
  CriaStr(Resultado, 29);
  Molda('9', '0', 1, 1, True, Resultado);
  Molda(IntToStr(Cont0), '0', 2, 8, True, Resultado);
  Molda(IntToStr(Cont1), '0', 9, 15, True, Resultado);
  Molda(IntToStr(NumVol), '0', 16, 22, True, Resultado);
  Molda(IntToStr(QuantVol), '0', 23, 29, True, Resultado);
  Result:= Resultado;
end;
