...
begin
  ...
  begin
    ...
    SQL.Sql.Add('Select OBSERVACAO from Forneced ');
...
//    Sql.ParamByName('CodGrupoSub').AsString:= GetProGrupo(Cds_MovCliCODPRODUTO).AsString;
    ...
    begin
      Quant:= Quant+ Cds_MovCliQUANTATENDIDA.AsFloat;
      Cds_MovCli.Next;
    end;
...
begin
  ...
  begin
    ...
    begin
      Quant:= Quant+ Cds_MovForQUANT.AsFloat;
      Cds_MovFor.Next;
    end;
...
//    Data:= EncodeDate(Ano, Mes, Dia);
    ...
    begin
      TotalVol:= TotalVol +
      Round((SQl.FieldByName('SaldoAtual').AsFloat / SQl.FieldByName('UnidadeEstoque').AsInteger)
...
begin
  ...
  begin
    Sql.Sql.Clear;
    Sql.Sql.Add('select MCPro.QuantAtendida, P.CodProduto, P.UnidadeEstoque, P.Cubagem, P.Peso '  +
    ...
    TotalPeso:= TotalPeso+(PesoEstIni + (PesoFimEnt-TotalPeso)-PesoEstFim);
    TotalVol:= TotalVol+(VolEstIni + (VolFimEnt-TotalVol)-VolEstFim);
...
begin
  ...
  begin
    ...
    ProFim:= CodPro;
    VolFimEnt:= TotalVol; PesoFimEnt:= TotalPeso;
...
begin
  Movimento(IncCont, 1011998, 0, '0', '0', '0', '0', ProFim, '0', VolFimEnt, PesoFimEnt, EndOfTheMonth(Ini));
end;
...
begin
  try
    ...
    AssignFile(vArq, Edit1.Text + '\ANP.txt');
