...
uses
  SysUtils, Classes, DBXpress, FMTBcd, SqlExpr, DB, Provider, DBClient, IniFiles, Forms;
...
type
  TDtm_Notas = class(TDataModule)
    ...
    function GetProObs(Codigo: String): String;    
    ...
    procedure GetCliDados(Codigo: String; var CGCMF, Nome: String);
  private
...
procedure TDtm_Notas.GetCliDados(Codigo: String; var CGCMF, Nome: String);
begin
  SQL.Close;
  SQL.SQL.Clear;
  SQL.SQL.Add('Select NUMEROCGCMF, NomeCliente from CLIENTE ');
  SQL.SQL.Add('where CodCliente =:Cod');
  SQL.ParamByName('Cod').AsString:= Codigo;
  SQL.Open;
  CGCMF:= Trim(SQL.FieldByName('NUMEROCGCMF').AsString);
  Nome:= Trim(SQL.FieldByName('NomeCliente').AsString);
end;
...
function TDtm_Notas.GetProObs(Codigo: String): String;
begin
  SQL.Close;
  SQL.SQL.Clear;
  SQL.SQL.Add('Select Observacao from Produto');
  SQL.SQL.Add('where CodProduto = :Cod');
  SQL.ParamByName('Cod').AsString:= Codigo;
  SQL.Open;
  Result:= Trim(SQL.FieldByName('Observacao').AsString);
end;

function TDtm_Notas.GetProVol(Codigo: String; Quant: Real): Double;
begin
  SQL.Close;

  SQL.Sql.Clear;

  SQL.Sql.Add('Select cubagem, unidadeEstoque from Produto');
  SQL.Sql.Add('where CodProduto =:Cod');
  ...
  Result:= (Quant/SQL.FieldByName('UnidadeEstoque').AsFloat)*SQL.FieldByName('cubagem').AsFloat*1000;
end;

function TDtm_Notas.GetProPeso(Codigo: String; Quant: Real): Double;
begin
  SQL.Close;

  SQL.Sql.Clear;

  SQL.Sql.Add('Select Peso, unidadeEstoque from Produto');
  SQL.Sql.Add('where CodProduto =:Cod');
  ...
  Result:= (Quant/SQL.FieldByName('UnidadeEstoque').AsFloat)*SQL.FieldByName('peso').AsFloat;
end;
...
begin
  if FileExists(ExtractFilePath(Application.ExeName) + 'Banco.Ini') then
    ...
    SQLConnection.Params.Values['Database']:= 'C:\Sidicom.new\Dados\Banco.gdb';
