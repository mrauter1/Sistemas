...
type
  ...
  TFrmConsultaPersonalizada = class(TForm)
    ...
    procedure cxGridChartViewCustomDrawLegendItem(Sender: TcxGridChartView;
...
begin
  try
    ...
    MemoSQL.Clear;
    MemoSQL.Lines.Add(FDm.SqlOriginal);
...
begin
  ...
  cxGridTabelaDBTableView1.StoreToIniFile(FNomeIni, False);
  DBPivotGrid.StoreToIniFile(FNomeIni, False);
  cxGridChartView.StoreToIniFile(FNomeIni, False);
...
begin
  ...
  cxGridTabelaDBTableView1.RestoreFromIniFile(FNomeIni, False);
  DBPivotGrid.RestoreFromIniFile(FNomeIni, False);
  cxGridChartView.RestoreFromIniFile(FNomeIni, False);
...
begin
  ...
  begin
    ...
    if SaveDialog.Execute then
      begin
        ExportGridToExcel(SaveDialog.FileName,cxGridTabela, True,True,True,'xls');
...
procedure TFrmConsultaPersonalizada.cxGridChartViewCustomDrawLegendItem(
...
begin
  ...
  begin
    ...
    begin
      if not fdm.QryCamposCor.IsNull then
        Result := AvalueCor;
    end;
