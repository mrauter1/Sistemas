...
type
  TFormActivityEditor = class(TForm)
    PanelTop: TPanel;
    PanelCentro: TPanel;
...
procedure TFormActivityEditor.AbrirConfig(pAtividade: TActivity);
begin
...
function TFormActivityEditor.NovaAtividade: TActivity;
begin
...
class procedure TFormActivityEditor._AbrirConfigAviso(pAtividade: TActivity);
var
  FFrm: TFormActivityEditor;
begin
  FFrm:= TFormActivityEditor.Create(Application);
  try
...
class function TFormActivityEditor._NovaAtividade: TActivity;
var
  FFrm: TFormActivityEditor;
begin
  FFrm:= TFormActivityEditor.Create(Application);
...
procedure TFormActivityEditor.BtnConfiguraProcessoClick(Sender: TObject);
begin
  ConfigurarProcesso;
end;

procedure TFormActivityEditor.BtnExecutarClick(Sender: TObject);
begin
  FActivity.Executar;
end;

procedure TFormActivityEditor.BtnAddProcessoClick(Sender: TObject);
var
  FIDConsulta: Integer;
...
function TFormActivityEditor.ProcessoSelecionado: TProcessoBase;
begin
...
function TFormActivityEditor.ProcessoDao: IDaoBase;
begin
  Result:= FActivityDao.ChildDaoByPropName('Processos');
end;

constructor TFormActivityEditor.Create(AOWner: TComponent);
begin
  inherited;
...
destructor TFormActivityEditor.Destroy;
begin
...
procedure TFormActivityEditor.FieldGetText(Sender: TField;
  var Text: string; DisplayText: Boolean);
begin
  Text:= Sender.AsString;
end;

procedure TFormActivityEditor.FieldSetText(Sender: TField;
  const Text: string);
begin
  Sender.AsString:= Text;
end;

procedure TFormActivityEditor.FormCloseQuery(Sender: TObject;
...
procedure TFormActivityEditor.BtnFecharClick(Sender: TObject);
begin
  Close;
end;

procedure TFormActivityEditor.BtnMoveDownClick(Sender: TObject);
var
  FCurIndex: Integer;
begin
  if ProcessoSelecionado=nil then
...
procedure TFormActivityEditor.BtnMoveUpClick(Sender: TObject);
var
  FCurIndex: Integer;
begin
  if ProcessoSelecionado=nil then
...
procedure TFormActivityEditor.RemoverProcesso;
begin
  if ProcessoSelecionado = nil then
    Exit;
...
procedure TFormActivityEditor.ActivityBeforePost(DataSet: TDataSet);
const
...
procedure TFormActivityEditor.BtnRemoveProcessoClick(Sender: TObject);
begin
  RemoverProcesso;
end;

procedure TFormActivityEditor.BtnSalvarClick(Sender: TObject);
begin
